...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (isFirstVisit) {
    ...
    const firstVisitTime = Number.parseInt(
      localStorage.getItem("firstVisitTime") || "0"
    );
  ...
  if (isFirstVisit) {
    showNotification(
      "Welcome to Ratatouille! Enjoy a special 15% discount on your first order.",
      "success"
    );
  }
});
...
function initializeMenu() {
    ...
    menuItems = [
      {
        ...
        description:
          "Crisp romaine lettuce with our homemade Caesar dressing, croutons, and parmesan cheese.",
        price: 250,
        image:
      ...
      },
      {
        ...
        price: 180,
        image:
          "https://www.thecomfortofcooking.com/wp-content/uploads/2018/10/Cheesy_Pull_Apart_Garlic_Bread-2.jpg",
        category: "starters",
      },
      {
        id: 3,
      ...
      },
      {
        ...
        description:
          "Grilled chicken in a creamy tomato sauce, served with rice.",
        price: 420,
        image:
    ...
    if (refreshCount >= refreshThreshold) {
      ...
      menuItems.forEach((item) => {
        ...
        item.price = Math.round(
          item.price * (1 + priceIncreasePercentage / 100)
        );
      });
  ...
  vipMenuItems = [
    ...
    },
    {
      ...
      description:
        "Lobster cooked in a rich, creamy sauce with mushrooms and cognac.",
      price: 1500,
      image:
...
function initializeTables() {
  ...
  const occupiedCount = Math.floor(
    Math.random() * (totalTables * 0.4) + totalTables * 0.3
  );
  ...
  for (let i = 1; i <= totalTables; i++) {
    ...
    tableElement.className = `table ${
      occupiedTables.includes(i) ? "occupied" : "available"
    }`;
  ...
  if (occupiedTables.length === totalTables) {
    ...
    bookingMessage.textContent =
      "All tables are currently occupied. You are on the waiting list.";
...
function applyTimePricing() {
  ...
  // Evening random discounts (7 PM - 10 PM)
    ...
    for (let i = 0; i < 2; i++) {
      ...
      menuItems[itemIndex].price = Math.round(
        menuItems[itemIndex].originalPrice * (1 - discountPercent / 100)
      );
  ...
  if (refreshCount > 5 && Date.now() - lastRefreshTime < 60000) {
    ...
    showNotification(
      "Anti-bot measure activated: Prices increased by 5% due to frequent page refreshes.",
      "warning"
    );
  }
...
function updateTimeDisplay() {
  ...
  document
    .querySelectorAll("#current-time-display, #menu-time-display")
    .forEach((el) => {
  ...
  if (hour >= 8 && hour < 11) {
    ...
    timeMessage =
      "Late Night Surge: All prices are 25% higher for last orders.";
  } else {
...
function setupEventListeners() {
  ...
  if (proceedToPaymentBtn) {
    proceedToPaymentBtn.addEventListener("click", () => {
      if (!tableBooked) {
        showNotification(
          "Please book a table before proceeding to payment.",
          "error"
        );
  ...
  if (mysteryLink) {
    mysteryLink.addEventListener("click", (e) => {
      ...
      showNotification(
        "Congratulations! You found the mystery discount. 20% off your next order!",
        "success"
      );
...
function renderMenu(filter) {
  ...
  if (cartTotal >= 1000 && !vipMenuUnlocked) {
    ...
    showNotification(
      "Congratulations! You've unlocked the exclusive VIP menu!",
      "success"
    );
  }
...
function addToCart(itemId, isVip = false) {
  // Find the item
  const item = isVip
    ? vipMenuItems.find((item) => item.id === itemId)
...
function updateCartItemQuantity(itemId, change, isVip) {
  const itemIndex = cart.findIndex(
    (item) => item.id === itemId && item.isVip === isVip
  );
...
function applyDiscount() {
  ...
  if (discountCode === "new" && isFirstVisit) {
    ...
    discountMessage.textContent =
      "New user discount (15% off) applied successfully!";
    discountMessage.className = "message success";
  } else if (
    discountCode === "mystery" &&
    localStorage.getItem("mysteryDiscount") === "true"
  ) {
    discountApplied = true;
    discountMessage.textContent =
      "Mystery discount (20% off) applied successfully!";
    discountMessage.className = "message success";
  } else {
...
function selectTable(tableNumber) {
  ...
  if (blockedUntil && Date.now() < blockedUntil) {
    ...
    showNotification(
      `Booking is temporarily blocked. Please try again in ${remainingTime} minutes.`,
      "error"
    );
    return;
  }
  ...
  if (lastBookingAttempt > 0 && now - lastBookingAttempt < 30000) {
    ...
    showNotification(
      "Too many booking attempts. Booking is blocked for 5 minutes.",
      "error"
    );
    return;
  }
  ...
  const selectedTableDisplay = document.getElementById(
...
function bookTable() {
  ...
  // Show notification
  showNotification(
    "Table booked successfully! You can now proceed to order.",
    "success"
  );
}
    ...
    if (paymentTimeLeft <= 0) {
      ...
      if (paymentMessage) {
        paymentMessage.textContent =
          "Session expired. Your cart has been reset.";
...
function renderPaymentSummary() {
  const paymentItemsContainer = document.getElementById(
...
setInterval(() => {
  ...
  if (hour >= 19 && hour < 22) {
    ...
    for (let i = 0; i < 2; i++) {
      ...
      menuItems[itemIndex].price = Math.round(
        menuItems[itemIndex].price * (1 - discountPercent / 100)
      );
      menuItems[itemIndex].discount = discountPercent;
    }
    ...
    if (
      ...
      renderMenu(activeFilter.dataset.filter);
      showNotification(
        "Evening special discounts have been updated!",
        "success"
      );
    }
  }
